...
export default function Hero() {
    return (
        <section className="relative w-full bg-neutral-900 flex items-center justify-center px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16 min-h-screen overflow-hidden">
            ...
            <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-16 items-center relative z-10m max-sm:mt-20">
                {/* Left Column: Text Content */}
                <div className="space-y-6 sm:space-y-8 text-center lg:text-left">
                    {/* Main Heading */}
                    <div className="space-y-3 sm:space-y-4">
                        <div className="overflow-hidden">
                            <h1 className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white leading-tight sm:leading-tight opacity-100 translate-y-0">
                                Find Your{' '}
                                <span className="text-teal-400 relative inline-block">
                                    Dream Apartment
                                    <div className="absolute -bottom-1 sm:-bottom-2 left-0 w-full h-0.5 bg-teal-400/30 scale-x-100"></div>
                                </span>
                            </h1>
                        </div>

                        <div className="overflow-hidden">
                            <p className="text-xl sm:text-2xl lg:text-3xl text-gray-300 font-light opacity-100 translate-y-0">
                                Anywhere, Anytime
                            </p>
                        </div>
                    </div>
                    ...
                    {/* Description */}
                    <div className="overflow-hidden">
                        <p className="text-gray-400 text-base sm:text-lg lg:text-xl max-w-lg mx-auto lg:mx-0 leading-relaxed opacity-100 translate-y-0">
                    ...
                    <div className="flex flex-wrap justify-center lg:justify-start gap-4 sm:gap-6 lg:gap-8 pt-2 sm:pt-4">
                        {[
                            { icon: faHome, value: "10K+", label: "Premium Properties" },
                            { icon: faCity, value: "50+", label: "Cities Worldwide" },
                    ...
                    {/* CTA Buttons */}
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center lg:justify-start pt-4 sm:pt-6 opacity-100 translate-y-0">
                ...
                <div className="grid grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
                    {[
                        { icon: faHome, title: "Luxury Apartments", desc: "Premium comfort & style" },
                        { icon: faMapMarkerAlt, title: "Prime Locations", desc: "Best neighborhoods" },
                        { icon: faStar, title: "Top Rated", desc: "Verified reviews" },
                        { icon: faHeart, title: "Favorite Picks", desc: "Most loved" }
                    ].map((feature, index) => (
                        <div
                            key={feature.title}
                            className="bg-neutral-800/30 backdrop-blur-lg rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 border border-gray-700/50 hover:border-teal-400/50 transition-all duration-500 transform hover:scale-105 cursor-pointer group relative overflow-hidden opacity-100 translate-y-0"
                        >
