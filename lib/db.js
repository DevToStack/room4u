  ...
  try {
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS booking_records (
        ...
        -- USER DETAILS (legal + safe)
        user_id INT NOT NULL,
        username VARCHAR(150) NOT NULL,
        user_email_hash CHAR(64) NOT NULL,        -- hashed email for privacy + fraud prevention
        phone_hash CHAR(64) NOT NULL,             -- hashed phone
        ...
        -- LEGAL BOOKING DATA (safe to store long-term)
        apartment_id INT,
        
        -- PAYMENT LEGAL LOGS
        last_payment_amount DECIMAL(10,2),
        last_payment_status ENUM('paid','pending','refunded'),
    
        -- IMPORTANT LEGAL TIMESTAMPS
        account_created_at DATETIME,
        account_deleted_at DATETIME NULL,
        
        -- NOTES FOR FRAUD / DISPUTES
        admin_notes TEXT,
    
        -- INDEXES FOR SPEED
        INDEX (user_id),
        INDEX (user_email_hash),
        INDEX (phone_hash),
        INDEX (apartment_id)
    );
    `);
    ...
    //   reviewer_id INT DEFAULT NULL, --Admin who reviewed
